...
async function main() {
  ...
  // -------------------------------------------------------------------------
  // 3.3 导入社区数据 (全市住区合并.json, WGS84 原值, 不做坐标转换)
  //     数据源: 高德住区多边形 + 房天下住户信息合并
  //     经对比验证: JSON 坐标系为 WGS84 (与 communities.csv WGS84 坐标差异<10m)
  //     住户字段: 徐州小区_9 (如 "3326户"), 面积: area_1
  // -------------------------------------------------------------------------
  const communitiesJsonPath = path.join(process.cwd(), "data", "全市住区合并.json");
  ...
  // 从地址推断行政区
  function inferDistrict(addr: string, adname: string): string {
    if (adname && adname.trim()) return adname.trim();
    if (!addr) return "未知";
    const districts = ["泉山区", "云龙区", "鼓楼区", "铜山区", "贾汪区", "沛县", "邳州市", "睢宁县", "新沂市", "丰县"];
  ...
  // 解析户数字段 (如 "3326户" → 3326, "118户" → 118)
  function parseHouseholds(text: string): number {
    if (!text) return 0;
  ...
  for (let i = 0; i < communityFeatures.length; i++) {
    ...
    // 提取名称: 优先 name_1 / 徐州小区1_, 其次 name
    const name = (props.name_1 || props["徐州小区1_"] || props.name || "").trim();
    if (!name) {
      skippedCount++;
      continue;
    }
    ...
    // 提取行政区
    const district = inferDistrict(props.address_1 || "", props.adname || "");

    // 提取地址
    const address = (props.address_1 || props.address || "").trim();

    // 提取面积 (平方米)
    const areaGis = Number(props.area_1 || props.area) || 0;

    // 提取户数: 徐州小区_9 (如 "3326户")
    let householdCount = parseHouseholds(props["徐州小区_9"] || "");

    // 提取建筑类型
    const buildingType = (props["徐州小区_6"] || props["徐州小区_5"] || "").trim();

    // 处理几何: Polygon 或 MultiPolygon → 取最大 Polygon
    ...
    // 坐标系为 WGS84, 直接使用原始顶点 (不做坐标转换)
    const wgsCoords = polygonCoords.map(([glng, glat]) => [glng, glat] as [number, number]);

    // 人口直接使用住户数 (一户算一人, 不再做 ×3.2 估算)
    // householdCount 已在上方从徐州小区_9 提取, 此处补充按面积估算
    if (householdCount <= 0 && areaGis > 0) {
      householdCount = Math.round(areaGis / 120);
    } else if (householdCount <= 0) {
      const calcArea = polygonArea(polygonCoords);
      if (calcArea > 0) {
        householdCount = Math.round(calcArea / 120);
      }
    }
    // 人口 = 住户数
    let population = householdCount;

    // 限制异常值
    if (population > 50000) population = 50000;
    if (householdCount > 15000) householdCount = 15000;

    // 构建 POLYGON WKT (MySQL 9.x SRID 4326 坐标轴顺序: lat lng)
    // 确保多边形闭合
    ...
    try {
      await conn.execute(
        `INSERT INTO t_community
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ST_GeomFromText(?, 4326))`,
        [communityCode, name, district, "未知街道", population, householdCount, areaGis, polygonWkt]
      );
      communityCount++;
    } catch (err: any) {
